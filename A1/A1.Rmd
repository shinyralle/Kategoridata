...
Gör vi dessa uträkningar så får vi stickprovsoddskvoten `r stickprovsoddskvot`. Under antagande att enkätdatan är multinomialfördelad gäller att standardefelet för oddskvoten kan estimeras med  
...
vanster_andpunkt <- exp(log(stickprovsoddskvot) + qnorm(0.975) * se_logstickprovsoddskvot)
hoger_andpunkt <- exp(log(stickprovsoddskvot) + qnorm(0.025) * se_logstickprovsoddskvot)

```

standardfelet för $\widehat{\sigma}(\text{stickprovsoddskvot})$ är `r se_logstickprovsoddskvot` och att konfidensintervallet för blir (`r vanster_andpunkt`,`r hoger_andpunkt`). 
